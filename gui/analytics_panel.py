...
class LatencyDiagnosticsPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.lbl_one_way = QLabel("Estimated One-Way: --")
        self.lbl_rtt_jitter = QLabel("RTT Jitter: --")
    ...
    def update_values(self, rtt_ms: float | None, one_way_ms: float | None, jitter_ms: float | None, gui_to_motion_ms: float | None, tx_to_motion_ms: float | None) -> None:
        def text(value: float | None) -> str:
            return "--" if value is None else f"{value:.1f} ms"
        self.lbl_cri_rtt.setText(f"CRI ACK RTT: {text(rtt_ms)}")
        self.lbl_one_way.setText(f"Estimated One-Way: {text(one_way_ms)}")
        self.lbl_rtt_jitter.setText(f"RTT Jitter: {text(jitter_ms)}")
