...
class CRIProtocolParser:
    ...
    def _parse_status(self, parameters: list[str]) -> None:
        ...
        with self.robot_state_lock:
            self.robot_state.status_fields_seen = set()

        while segment_start_idx < len(parameters):
            match parameters[segment_start_idx]:
                case "MODE":
                    with self.robot_state_lock:
                        self.robot_state.status_fields_seen.add("MODE")
                ...
                case "SUPPLY":
                    val = int(parameters[segment_start_idx + 1])
                    with self.robot_state_lock:
                        self.robot_state.status_fields_seen.add("SUPPLY")
                        self.robot_state.supply_voltage = float(val)
                    segment_start_idx += 2

                case "CURRENTALL":
                    val = int(parameters[segment_start_idx + 1])
                    with self.robot_state_lock:
                        self.robot_state.status_fields_seen.add("CURRENTALL")
                        self.robot_state.current_total = float(val)
                    segment_start_idx += 2

                case "CURRENTJOINTS":
                    currents = []
                    for i in range(16):
                        currents.append(float(int(parameters[segment_start_idx + 1 + i])))

                    with self.robot_state_lock:
                        self.robot_state.status_fields_seen.add("CURRENTJOINTS")
